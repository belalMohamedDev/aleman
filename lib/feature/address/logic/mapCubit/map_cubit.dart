...
class MapCubit extends Cubit<MapState> {
  ...
  Future<void> moveToLocationInTextFormField(MapBoxPlace place) async {
    ...
    try {
      ...
      final double lat = place.geometry?.coordinates.lat ?? 0.0;
      final double lng = place.geometry?.coordinates.long ?? 0.0;
...
// Check if the address is available
  ...
  Future<void> getCurrentLocation(BuildContext context) async {
    ...
    try {
      final Position? position = await _determinePosition(context).timeout(
  ...
  void addLocationToMap(BuildContext context) {
    ...
    try {
      ...
      final LatLng currentPosition = LatLng(lat, long);
